...
from mdg.confluence.util import basic_auth, http_post, http_get_json
...
def update_images(auth, content_id, path):
    ...
    for file in files:
        ...
        if conf_attachment is not None:
            ...
            url = "https://{}.atlassian.net/wiki/rest/api/content/{}/child/attachment/".format( ORG, content_id, attachment['id'] )
            http_post( url, auth, None, file, None )
